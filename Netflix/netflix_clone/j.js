//Const
const apikey="8a181e96c2de729a9d8e075fd5ff7a17"
const apiEndPoint="https://api.themoviedb.org/3"
const imgPath="https://image.tmdb.org/t/p/original";
const apiPaths={
    fetchAllCategories: `${apiEndPoint}/genre/movie/list?api_key=${apikey}`,
    fetchTrending:`${apiEndPoint}/trending/all/day?api_key=${apikey}&language=en-US`,
    fetchLatest:`${apiEndPoint}/movie/latest?api_key=${apikey}&language=en-US`,
    fetchNetflixOriginals:`${apiEndPoint}/discover/tv?api_key=${apikey}&with_network=213`,
    fetchTopRated:`${apiEndPoint}/movie/top_rated?api_key=${apikey}language=en-US`,
    fetchSimilarMoives:(id)=> `${apiEndPoint}/moive/${id}/similar?api_key=${apikey}&language=en-US`,
    fetchMoviesList: (id) =>`${apiEndPoint}/discover/movie?api_key=${apikey}&with_genres=${id}`,
    searchYoutube:(query) => `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${query}&key=AIzaSyAgzfp68wZ1iTBSlYn2BRWACxvRuND7D1g`
}

function init(){
    // fetch(apiPaths.fetchAllCategories)
        fetchTrendingMovies();
       fetchAllBuildAllCategories();
}
    ...
    function buildBannerSection(movie){
        ...
        <p class="banner_info">Trending in moives | Released-${movie.release_date}</p>
    ...
    function fetchAllBuildAllCategories(){
        fetch(apiPaths.fetchAllCategories)
        .then(res=>res.json())
        .then(res => {
...
function searchMovieTrailer(movieName,iframeId){
    if(!movieName)return;
    fetch(apiPaths.searchYoutube(movieName+'trailer'))
    .then(res=>res.json())
    .then(res=>{
